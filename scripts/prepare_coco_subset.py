...
# 检查文件是否存在
if not os.path.exists(coco_val_annotations_file):
    raise FileNotFoundError(f"文件不存在: {coco_val_annotations_file}")

# 输出路径
output_dir = '/content/few_shot_object_detection/data'
...
os.makedirs(os.path.join(val_output_dir, 'labels'), exist_ok=True)
os.makedirs(test_output_dir, exist_ok=True)
os.makedirs(os.path.join(test_output_dir, 'images'), exist_ok=True)
os.makedirs(os.path.join(test_output_dir, 'labels'), exist_ok=True)
...
# 创建日志文件
log_file_path = os.path.join(output_dir, 'invalid_labels_log.txt')
    ...
    def save_coco_subset(coco, image_ids, images_dir, output_dir):
        for img_id in image_ids:
            ...
            if not os.path.exists(src_image_path):
                log_file.write(f"文件不存在: {src_image_path}\n")
                continue
            ...
            with open(label_file_path, 'w') as f:
                for ann in anns:
                    ...
                    if category_id < 0 or category_id >= 80:
                        log_file.write(f"忽略无效标签: {category_id + 1} 在图像 {img_info['file_name']}\n")
                        continue
                    bbox = ann['bbox']
